...
exports.likePost = async (req, res, next) => {
  try {
    const { postId } = req.params;
    const { user } = req;
    const responce = await Friendship.postLiked(user._id, { postId });
    if (responce.ok) {
...
exports.unlikePost = async (req, res, next) => {
  try {
    const { postId } = req.params;
    const { user } = req;
    const responce = await Friendship.postUnliked(user._id, { postId });
    if (responce.ok) {
...
exports.unrepostPost = async (req, res, next) => {
  try {
    ...
    const { user } = req;
    assert.ok(user);
    const doc = await Post.findOne({ retweeted_status: post._id });
    await doc.deleteOne();
...
exports.getLikes = async (req, res, next) => {
  try {
    ...
    const users = await serializeUsers(doc.liked_by, req.user);
    res.json({ users });
  } catch (err) {
    next(err);
  }
...
exports.getReposts = async (req, res, next) => {
  try {
    ...
    if (!doc) { return res.json({ users: null }); }
    const users = await serializeUsers(doc.reposted_by, req.user);
    res.json({ users });
  } catch (err) {
    next(err);
  }
