...
exports.serializeNotif = async (notif, client) => {
  ...
  let user = null;
  if (notif.body.user) { user = await serializeUser(notif.body.user, client); }
  let post = null;
  if (notif.body.post) { post = await serializePost(notif.body.post, client); }
...
exports.serializeNotifs = async (doc) => {
  if (!doc instanceof Document) { throw Error('Unknown Notification object'); }
  doc = await doc
    .populate({
