...
friendshipSchema.statics.countFriends = async (user_id) => {
  const doc = await this.findOne({ user_id }, 'friend_ids');
  if (!doc || !doc.friend_ids) { return 0; }
  let { length } = doc.friend_ids;
  if (doc.friend_ids.includes(user_id)) { length--; }
  return length;
};
friendshipSchema.statics.countFollowers = async (user_id) => {
  const doc = await this.findOne({ user_id }, 'follower_ids');
  if (!doc || !doc.follower_ids) { return 0; }
  let { length } = doc.follower_ids;
 ...
 * @param {*} user2_id
 */
friendshipSchema.statics.isFollowing = async (user1_id = null, user2_id = null) => this.exists({
  user_id: user1_id,
  friend_ids: user2_id,
});
/**
 ...
 * @param {*} user2_id
 */
friendshipSchema.statics.isFollowed = async (user1_id = null, user2_id = null) => this.exists({
  user_id: user1_id,
 ...
 * @param {*} post_id
 */
friendshipSchema.statics.isLiked = async (user_id = null, post_id = null) => this.exists({
  user_id,
  liked_posts: post_id,
});
friendshipSchema.statics.isReposted = async (user_id = null, post_id = null) => this.exists({
  user_id,
  reposted_ids: post_id,
});

friendshipSchema.statics.postLiked = async (user_id = null, { post_id, postId }) => {
  if (postId) {
    const post = await mongoose.model('Post').findOne({ id_str: postId }, '_id');
    post_id = post._id;
  } else if (!post_id) { throw Error('Cannot determine post'); }
  const liked = await this.isLiked(user_id, post_id);
  if (liked) { return ({ ok: 1, nModified: 0 }); }
  const res1 = await this.updateOne({ user_id }, {
...
friendshipSchema.statics.postUnliked = async (user_id = null, { post_id, postId }) => {
  if (postId) {
    const post = await mongoose.model('Post').findOne({ id_str: postId }, '_id');
    post_id = post._id;
  } else if (!post_id) { throw Error('Cannot determine post'); }
...
friendshipSchema.statics.postReposted = async (user_id = null, { post_id, postId }) => {
  if (postId) {
    const post = await mongoose.model('Post').findOne({ id_str: postId }, '_id');
    post_id = post._id;
  } else if (!post_id) { throw Error('Cannot determine post'); }
  const reposted = await this.isReposted(user_id, post_id);
...
friendshipSchema.statics.postUnreposted = async (user_id = null, { post_id, postId }) => {
  if (postId) {
    const post = await mongoose.model('Post').findOne({ id_str: postId }, '_id');
    post_id = post._id;
  }
 ...
 * @param {*} user2_id
 */
friendshipSchema.statics.gotFollowed = async (user1_id = null, user2_id = null) => {
  const follower = await this.isFollowed(user1_id, user2_id);
 ...
 * @param {*} user2_id
 */
friendshipSchema.statics.gotUnfollowed = async (user1_id = null, user2_id = null) => {
  const follower = await this.isFollowed(user1_id, user2_id);
