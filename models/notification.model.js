...
notifySchema.statics.push = async (user_id, ...notifs) => {
  let doc = await this.findOne({ user_id });
  if (!doc) { doc = await this.create({ user_id }); }
  return doc.push(...notifs);
};
notifySchema.methods.push = async (...notifs) => {
  const maxSize = 7;
...
async function sendPushNotif(notif, subscriptions, user_id) {
  ...
  // Send via push
  let page; let body; let
    { title } = notif;
  if (notif.type === 'mentioned') {
    const post = await mongoose.model('Post').findById(notif.body.post).populate('user');
    ...
    const user = await mongoose.model('User').findById(notif.body.user);
    const post = await mongoose.model('Post').findById(notif.body.post);
    ...
    const reply_post = await mongoose.model('Post').findById(notif.body.post).populate('user');
