...
userSchema.statics.countPosts = async (user_id) => mongoose.model('Post').countDocuments({ user: user_id });
...
userSchema.methods.validPassword = async (password) => Auth.validPassword(this._id, password);
/**
 ...
 */
userSchema.methods.follow = async (...list_id_tobe_friends) => {
  let res = { ok: 0 };
  try {
    const res1 = await Friendship.updateOne({ user_id: this._id }, {
      $push: {
        friend_ids: {
    ...
    if (res1.ok) {
      ...
      for (const id of list_id_tobe_friends) {
        await home_timeline
...
userSchema.methods.unfollow = async (...list_id_tobe_not_friends) => {
  ...
  try {
    ...
    if (res1.ok) {
      ...
      for (const id of list_id_tobe_not_friends) {
        await home_timeline
...
userSchema.statics.searchUser = (query) => {
...
userSchema.statics.getSuggestions = async function ({
  ...
  const { friend_ids } = await Friendship.findOne({ user_id });
...
async function user_genId() {
  ...
  const { current_user_id } = await internal_setting.findOne({ ver: '1.0' }, 'current_user_id');
