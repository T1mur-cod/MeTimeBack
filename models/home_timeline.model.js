...
timelineSchema.statics.getTimeline = async ({
  ...
  const p = parseInt(page); // page/batch number
  ...
  /**
     ...
     */
  if (!user_id) {
    const { _id } = await mongoose.model('User')
 ...
 */
timelineSchema.statics.bulkAddPosts = async (user_ids, ...args) => {
  const timelines = await this.find({ user_id: { $in: user_ids } });
...
*/
timelineSchema.methods.bulkAddPosts = async ({
  ...
  posts_toadd = posts_toadd.map((obj) => {
    const { _id: post_id, created_at } = obj;
    return {
      post_id,
      created_at,
    };
 ...
 * @param {Array} user_id - array of user._id's of concerned users
 */
timelineSchema.statics.bulkRemovePosts = async (user_ids, ...args) => {
  const timelines = await this.find({ user_id: { $in: user_ids } });
  timelines // forEach does parallelly (unlike for of)
...
* @returns res - result of update command
*/
timelineSchema.methods.bulkRemovePosts = async ({
  ...
  const ids_to_remove = posts_to_remove.map((pst) => pst._id);
  const res = await this.update({
    $pull: {
      posts: {
