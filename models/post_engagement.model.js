...
engageSchema.statics.gotLiked = async (post_id, user_id) => {
  if (await this.exists({ post_id, liked_by: user_id })) { return; }
...
engageSchema.statics.gotUnliked = async (post_id, user_id) => {
  if (!await this.exists({ post_id, liked_by: user_id })) { return; }
...
engageSchema.statics.gotReposted = async (post_id, user_id) => {
  if (await this.exists({ post_id, reposted_by: user_id })) { return; }
  const post = await mongoose.model('Post').findById(post_id);
  await post.updateOne({
    $inc: {
      retweet_count: 1,
...
engageSchema.statics.gotUnreposted = async (post_id, user_id) => {
  if (!await this.exists({ post_id, reposted_by: user_id })) { return; }
  await mongoose.model('Post').updateOne({ _id: post_id }, {
    $inc: {
      retweet_count: -1,
...
engageSchema.statics.gotReplied = async (post_id, reply_post_id) => {
  if (await this.exists({ post_id, reply_posts: reply_post_id })) { return; }
  const post = await mongoose.model('Post').findById(post_id);
  await Notification.push(post.user, { // user is not populated
    type: 'replied',
